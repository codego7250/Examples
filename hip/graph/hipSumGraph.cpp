// change from hip graph examples
...
__global__ void sum(float* d_in, double* d_out, size_t inputSize, size_t outputSize) {
...
__global__ void sumFinal(double* d_in, double* d_out, size_t inputSize) {
  int myId = threadIdx.x + blockDim.x * blockIdx.x;
  int tid = threadIdx.x;
  for (unsigned int s = blockDim.x / 2; s > 0; s >>= 1) {
    if (tid < s) {
...
bool hipWithoutGraphs(float* inputVec_h, float* inputVec_d, double* outputVec_d, double* result_d,
  ...
  for (int i = 0; i < GRAPH_LAUNCH_ITERATIONS; i++) {
    ...
    hipLaunchKernelGGL(sum, dim3(inputSize / THREADS_PER_BLOCK, 1, 1),
    ...
    hipLaunchKernelGGL(sumFinal, dim3(1, 1, 1), dim3(THREADS_PER_BLOCK, 1, 1), 0, stream1,
...
void myHostNodeCallback(void* data) {
  ...
  if (iter == GRAPH_LAUNCH_ITERATIONS)
    printf("[%s] Host callback final sum total = %lf\n", function, *result);
  result_gpu = *result;
...
bool hipGraphsManual(float* inputVec_h, float* inputVec_d, double* outputVec_d, double* result_d,
  ...
  kernelNodeParams.func = (void*)sum;
  ...
  kernelNodeParams.func = (void*)sumFinal;
